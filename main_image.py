...
def get_args_parser():
    ...
    # fulltune.1
    parser.add_argument('--merge_before_finetune', default=False, action='store_true')
    parser.add_argument('--mask_dict', default='', type=str)
    # fulltune.2
    parser.add_argument('--tune_value_only', default=False, action='store_true')
    # lora
...
def main(args):
    ...
    if args.model.startswith('vit'):
        # fine-tuning configs
        _d_model_dict = dict(base=768, large=1024, huge=1280)
        _d_model = next(v for k, v in _d_model_dict.items() if k in args.model)
        # input(str(_d_model))
        tuning_config = EasyDict(
            ...
            # Our method rescale/ mased finetune
            learn_scale=args.learn_scale,
            merge_before_finetune = args.merge_before_finetune,
        )
    ...
    if args.finetune and not args.eval:
        ...
        if args.merge_before_finetune:
            mask_dict = torch.load(args.mask_dict)
            def get_hook(mask):
                def hook(grad):
            ...
            for name, p in model.named_parameters():
                if name in mask_dict.keys():
                    flag = False
                    hook = get_hook(mask_dict[name].to(p.device))
                    p.register_hook(hook)
            if flag:
                raise ValueError("No finetune hook found")
    ...
    if args.use_lora:
        ...
        config = LoraConfig(
            r=2,
            lora_alpha=16,
            target_modules=["q_proj", "v_proj", "k_proj", "proj", "fc1", "fc2"],
            lora_dropout=0.05,
            bias="none",
            modules_to_save=["head"], # PEFT 会自动确保 head 是可训练的并包含在 checkpoint 中
        )
        ...
        for name, p in model.named_parameters():
            ...
            # else:
                ...
                if args.tune_value_only:
                    if "fc2" in name or "attn.proj" in name or "attn.v_proj" in name:
                        p.requires_grad = True
                    else:
    ...
    print(f"{len([p for name, p in model.named_parameters() if (p.requires_grad and 'scale' in name)])} scale param")
    optimizer = torch.optim.SGD([{'params': [p for name, p in model.named_parameters() if (p.requires_grad and "scale" in name)], 'lr_scale': 0.005},  # 第一层
        {'params': [p for name, p in model.named_parameters() if (p.requires_grad and "scale" not in name)], 'lr_scale': 1.}], lr=args.lr, weight_decay=args.weight_decay, momentum=0.9)
    print(optimizer)
